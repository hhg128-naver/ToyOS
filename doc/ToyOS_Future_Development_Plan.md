...
### 2단계: 프레임버퍼 기반 텍스트 렌더링 (그래픽 콘솔)
- **배경**: UEFI 환경의 GOP(Graphics Output Protocol)는 레거시 VGA 텍스트 모드(`0xB8000` 직접 접근)를 지원하지 않으므로, 픽셀 단위로 직접 글자를 그려야(Rendering) 합니다.
- **수행 항목**:
  - 내장 비트맵 폰트(예: 8x16 배열 폰트 데이터) 추가.
  - 주어진 문자를 픽셀로 변환하여 프레임버퍼에 쓰는 `PutChar(char c, int x, int y, uint32_t color)` 구현.
  - 기존 `Printf` 호환 커스텀 그래픽 콘솔 기능 작성 (개행 문자 `\n` 등 화면 스크롤 처리 포함).
