...
{
	/* 새로운 PML4 생성 */
	kernel_pml4 = static_cast<pt_entry*>(PMM_AllocPage());
	if (!kernel_pml4)
	{
		return;
	}
	 ...
	 */
	uint64_t framebuffer_base = (uint64_t)binfo->framebuffer;
	uint64_t framebuffer_size = (uint64_t)binfo->horizontal_resolution * binfo->vertical_resolution * 4;
	for (uint64_t addr = framebuffer_base; addr < framebuffer_base + framebuffer_size; addr += PAGE_SIZE)
	{
		VMM_MapPage((void*)addr, (void*)addr, PAGE_PRESENT | PAGE_WRITABLE);
	}
...
{
	uint64_t virtualAddress = (uint64_t)virtual_addr;
	uint64_t physicalAddress = (uint64_t)physical_addr;
	...
	else
	{
		...
		if ((flags & PAGE_USER) && !(pml4[pml4_idx] & PAGE_USER))
		{
			...
			if (new_table)
			{
				for (int i = 0; i < 512; i++)
				{
					((pt_entry*)new_table)[i] = ((pt_entry*)old_table)[i];
				}
	...
	pt[pt_idx] = physicalAddress | flags | PAGE_PRESENT;
}
...
{
	if (!pml4_phys || pml4_phys == kernel_pml4)
		return;
